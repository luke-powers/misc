...
class TextractWrapper:
    ...
    def get_detection_job(self, job_id, next_token=None):
        """
        Gets data for a previously started text detection job.

        :param job_id: The ID of the job to retrieve.
        :param next_token: The NextToken indicating the next block of data to retrieve.
...
def main():
    ...
    bucket = "pdfai-textract-bucket"
    document = "mckinsey china consumer report 2021.pdf"
    text_extractor = TextractWrapper(
        client, boto3.resource("s3"), boto3.resource("sqs")
    )
    job_id = text_extractor.start_detection_job(
        bucket,
        document,
        "arn:aws:sns:us-east-1:450870687564:textract-status",
        "arn:aws:iam::450870687564:role/SNSSuccessFeedback",
    )
    status = text_extractor.get_detection_job(job_id)["JobStatus"]
    dots = ""
    while status == "IN_PROGRESS":
        print(f"{job_id} - {status}{dots}", end="\r")
        time.sleep(1)
        if dots != "....":
            dots += "."
        else:
            status = text_extractor.get_detection_job(job_id)["JobStatus"]
            print(" " * (len(job_id) + len(status) + len(dots) + 3), end="\r")
            dots = ""
    end_time = time.time()
    ...
    ipdb.set_trace()
    print(text_extractor.get_detection_job(job_id))
